...
class CompletionProxyScriptTests(unittest.TestCase):
    ...
    def setUp(self):
        self.test_output_root_directory = "/tmp/batect-bash-completion-tests"
    ...
    def test_complete_arguments(self):
        ...
        self.assertEqual(version_script_invocation_details["arguments"], "--generate-completion-script=bash")
    ...
    def run_completions_for(self, input, working_directory):
        stdout = self.run_bash_command('test_complete.bash "{}"'.format(input), working_directory).rstrip("\n")
    ...
    def run_two_completions(self, first_input, second_input, first_working_directory, second_working_directory=None):
        ...
        divider = "---DIVIDER---"
        command = 'cd "{}" && test_complete.bash "{}" && echo "{}" && cd "{}" && test_complete.bash "{}"'.format(
        ...
        stdout = self.run_bash_command(command, first_working_directory).splitlines()
    ...
    def run_bash_command(self, command, working_directory):
        command_line = ["bash", "-c", command]
